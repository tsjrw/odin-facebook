    ...
    describe 'liking post' do
        context "when not logged" do
            it 'should redirect' do 
                expect{ post post_likes_path(user_post.id)}.to change(user_post.users_liking, :count).by(0)
        ...
        context 'when logged' do
            it 'increase the number of likes of the post' do
                sign_in user_1
                expect{ post post_likes_path(user_post.id)}.to change(user_post.users_liking, :count).by(1)
            end
